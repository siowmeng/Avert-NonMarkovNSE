...
nse_label_map = {0: 'safe', 1: 'middle', 2: 'unsafe'}

def init_log_folder(logfile_path, domain, ptPath, actorObjMode, constrMode, init_lambda, seed, list_constrs, lambda_lr):
    
    rundate = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    # datetime_suffix = datetime.datetime.now().strftime('_%Y%m%d_%H%M%S')
    # file_path = './' + domain + datetime_suffix + '/'
    lambda_lr_str = "%0.4f" % lambda_lr
    file_path = logfile_path + domain + '/' + actorObjMode + '/' + constrMode + '/' + str(init_lambda) + '/' + lambda_lr_str + '/' + str(seed) + '/'
    ...
    with open(file_path + 'mode.log', 'w') as f:
        ...
        f.write("\n")
        for class_idx, sign, threshold in list_constrs:
...
def log_test_trajs(base_path, epoch, domain_dict, d_traj_nse, d_prop_nse, d_discRet_stats, d_undiscRet_stats):
    ...
    for class_idx in d_traj_nse.keys():
        ...
        for idx, traj in enumerate(d_traj_nse[class_idx]):
            ...
            #     #x, y = traj['location(x)'].values, traj['location(y)'].values
            #     x, y = traj['location1'].values, traj['location2'].values
            #     fig, ax = plt.subplots()
            #     ax.quiver(x[:-1], y[:-1], x[1:]-x[:-1], y[1:]-y[:-1], scale_units='xy', angles='xy', scale=1)
                
            #     # Resistance Zones
            #     for i in range(domain_dict['dec_centers'].shape[0]):                                
            #         ax.plot(domain_dict['dec_centers'][i][0], domain_dict['dec_centers'][i][1], 
            #                 color='red', marker='x', linestyle='dashed', linewidth=2, markersize=12)
                
            #     #ax.plot(1.5, 3.0, color='red', marker='x', linestyle='dashed', linewidth=2, markersize=12)
            #     #ax.plot(5.0, 4.5, color='red', marker='x', linestyle='dashed', linewidth=2, markersize=12)
                
            #     # Goal
            #     ax.plot(domain_dict['goal'][0], domain_dict['goal'][1], 
            #             color='red', marker='o', linestyle='dashed', linewidth=2, markersize=12)
            #     #ax.plot(8.0, 9.0, color='red', marker='o', linestyle='dashed', linewidth=2, markersize=12)
            #     ax.set_xlabel('X', fontsize=12)
            #     ax.set_ylabel('Y', fontsize=12)
                
            #     # NSE Zone
            #     nse_circle = plt.Circle((3.25, 3.75), 2, fill = False)
            #     ax.add_patch(nse_circle)
            #     fig.savefig(full_path_prefix + '.png')
...
def log_nse_trajs(base_path, domain_dict, d_traj_nse, d_prop_nse):
    
    file_path = base_path + domain_dict['domain_name'] + '/'
    pathlib.Path(file_path).mkdir(parents = True, exist_ok = True)
